  ...
  const featuredSpeakers = useMemo(
    () =>
      apiSpeakers.slice(0, 4).map((speaker, index) => ({
        name: speaker.fullName || speaker.name || "Speaker",
        image: speaker.photoUrl || defaultImages[index % defaultImages.length],
        bio:
          speaker.biography ||
          `${speaker.fullName || speaker.name || 'Our expert speaker'} explores ${speaker.topic || speaker.scientificTheme || 'conference themes'} and contributes to the community.`,
      })),
    [apiSpeakers]
  );
  ...
  const displayedSpeakers = useMemo(
    () =>
      apiSpeakers.map((speaker, index) => ({
        ...
        topic: speaker.topic || speaker.scientificTheme || "General",
        domains: [speaker.topic || speaker.scientificTheme || "General"],
      })),
    [apiSpeakers]
  );

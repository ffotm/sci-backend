...
const Program = () => {
  ...
  const sessions = apiAgenda.map((item, index) => ({
    ...
    description: item.theme?.label || item.description || "Conference session details.",
    date: item.startsAt ? new Date(item.startsAt).toLocaleDateString() : (item.dateLabel || "TBD"),
    time: item.startsAt ? new Date(item.startsAt).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" }) : (item.timeSlot || "TBD"),

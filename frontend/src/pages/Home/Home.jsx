...
const Home = () => {
  ...
  const scheduleByDay = useMemo(() => {
    const days = { 1: [], 2: [], 3: [], 4: [] };
    const speakerMap = Object.fromEntries(apiSpeakers.map((speaker) => [speaker._id || speaker.id, speaker]));
    const images = [sp1, sp2, sp3, sp4, sp5, sp6, sp7, sp8, sp9, sp10, sp11, sp12];

    apiAgenda.forEach((item, index) => {
      const speakerId = typeof item.speakerId === "object" ? item.speakerId?._id : item.speakerId;
      const speaker = item.speakerId && typeof item.speakerId === "object"
        ? item.speakerId
        : (speakerMap[speakerId] || {});
      const startsAt = item.startsAt ? new Date(item.startsAt) : null;
      const day = startsAt ? Math.min(4, Math.max(1, startsAt.getDay() || 1)) : (item.day || 1);
      days[day] = days[day] || [];
      days[day].push({
        img: speaker.photoUrl || images[index % images.length],
        name: speaker.fullName || speaker.name || `Speaker ${index + 1}`,
        time: startsAt ? startsAt.toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" }) : (item.timeSlot || "TBD"),
        title: item.sessionTitle || "Conference Session",
        desc: item.theme?.label || item.description || speaker.biography || "Session details coming soon.",
        date: startsAt ? startsAt.toLocaleDateString() : (item.dateLabel || `Day ${day}`),
        location: item.room || item.location || "Main Hall",
      });
    });
